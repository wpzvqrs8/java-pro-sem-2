...
public class Controller{
    ...
    public void divison(ActionEvent event){
        expression += "/ ";
    }
    public void equals(ActionEvent event){
        String[] data = expression.split(" ");
        ArrayDequeue<Double> numbers = new ArrayDequeue<>();
        ArrayDequeue<Integer> operators = new ArrayDequeue<>();
        for(int i =0; i< data.length;i++){
            if(data[i].isOperator){
                operators.add(Integer.parseInt(data[i]));
            }else{
